...
const persist: AtomEffect<any> = ({ onSet, setSelf, node }) => {
  const list = localStorage.getItem(node.key);
  if (list !== null) {
    setSelf(JSON.parse(list));
  }
  onSet((newShoppingList) => {
    if (newShoppingList instanceof DefaultValue)
      localStorage.removeItem(node.key);
    else localStorage.setItem(node.key, JSON.stringify(newShoppingList));
  });
};
