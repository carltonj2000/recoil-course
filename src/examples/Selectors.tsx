...
const eurSelector = selector<number>({
  key: 'eur',
  get: ({ get }) => {
    let usd = exchangeRate * get(usdAtom);
...
export const Selectors = () => {
  const [usd, usdSet] = useRecoilState(usdAtom);
  const [eur, eurSet] = useRecoilState(eurSelector);
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      <InputStack>
        <CurrencyInput label="usd" amount={usd} onChange={(x) => usdSet(x)} />
        <CurrencyInput label="eur" amount={eur} onChange={(x) => eurSet(x)} />
      </InputStack>
      <Commission />
    </div>
  );
